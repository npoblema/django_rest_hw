...
class Locator(object):
    ...
    def _get_project(self, name):
        """
        For a given lms_project, get a dictionary mapping available versions to Distribution
        instances.
    ...
    def score_url(self, url):
        ...
        for a given lms_project release.
        """
        t = urlparse(url)
    ...
    def split_filename(self, filename, project_name):
        """
        Attempt to split a filename in lms_project name, version and Python version.
        """
        return split_filename(filename, project_name)

    def convert_url_to_download_info(self, url, project_name):
        """
        See if a URL is a candidate for a download URL for a lms_project (the URL
            ...
            for ext in self.downloadable_extensions:
                if path.endswith(ext):
                    ...
                    if not t:  # pragma: no cover
                        logger.debug('No match for lms_project/version: %s', path)
                    else:
...
class SimpleScrapingLocator(Locator):
    ...
    def _process_download(self, url):
        """
        See if an URL is a suitable download for a lms_project.
        ...
        if data:
            for info in data.get('files', []):
                if info['ptype'] != 'sdist' or info['pyversion'] != 'source':
                    continue
                # We don't store summary in lms_project metadata as it makes
